...
public class FetchContent extends Configured implements Tool {
	...
	public static class MapClass extends Mapper<LongWritable, Text, Text, Text> {
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			Text val = new Text("\"category\":\""+ category + "\",\"content\":\"" + text.toString() + "\"");
			
			context.write(new Text("\"domain\":\"" + domain.substring(domain.indexOf(".")+1) + "\""), val);
	...
	public static class ReduceClass extends Reducer<Text, Text, Text, Text> {
		public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
			...
			Text text = new Text("{" + key.toString() + "," + result.toString() + "}");
			context.write(null, new Text(text.toString()));
		}
	}
